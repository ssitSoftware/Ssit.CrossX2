...
fragment float4 fragmentMain(VertexOut in [[stage_in]],
                              ...
                              sampler normalSamp [[sampler(1)]],
                              constant LightingUniforms &lights [[buffer(0)]])
{
    float4 texColor = tex.sample(samp, in.uv) * in.color;

    // Reconstruct a per-pixel normal from the tangent-space normal map using the
    // screen-plane tangent/binormal basis (both 2D, since geometry is flat-facing).
    float3 T = normalize(float3(in.tangent, 0.0));
    ...
    for (int i = 0; i < 8; i++)
    {
		...
		intensity = localCellShading > 0 ? floor(intensity * localCellShading) / localCellShading : intensity;
    ...
    lighting = globalCellShading > 0 ? floor(lighting * globalCellShading) / globalCellShading : lighting;
