...
/// </summary>
internal sealed unsafe class CrtSimEffect : ICrtSimEffect
{
    private readonly SdlGpuRenderer _renderer;
    private readonly CrtSimulationEffectParameters _parameters;
    ...
    public CrtSimEffect(SdlGpuRenderer renderer, CrtSimulationEffectParameters parameters = null)
    {
        _renderer = renderer;
        SDL_GPUTextureFormat targetFormat = SDL_GetGPUSwapchainTextureFormat(renderer.Device, renderer.Window);
        
        _parameters = parameters ?? new CrtSimulationEffectParameters();
        _device = renderer.Device;

        SDL_GPUShader* vertexShader = GpuShader.CreateFromEmbeddedResource(renderer, "Effects.Crt.vert", "vertexMain", SDL_GPUShaderStage.SDL_GPU_SHADERSTAGE_VERTEX, numUniformBuffers: 1);
        SDL_GPUShader* fragmentShader = GpuShader.CreateFromEmbeddedResource(renderer, "Effects.Crt.frag", "fragmentMain", SDL_GPUShaderStage.SDL_GPU_SHADERSTAGE_FRAGMENT, numSamplers: 1, numUniformBuffers: 1);
        ...
        _pipeline = CreatePipeline(_device, vertexShader, fragmentShader, targetFormat);
        ...
        _vertexBuffer = CreateAndUploadVertexBuffer(_device, copyPass, quadVertices);
    ...
    public void Render(SDL_GPUCommandBuffer* commandBuffer, SDL_GPUTexture* sourceTexture, int sourceWidth, int sourceHeight,
        SDL_GPUTexture* outputTexture, uint outputWidth, uint outputHeight, float sourceScale, SDL_FColor clearColor)
    {
    ...
    public void Render(IRenderTarget target, ITexture source, float sourceScale = 1)
    {
        _renderer.SubmitCommandBuffer();

        var sdlRenderTarget = (SdlGpuRenderTarget)target;
        var sdlSourceTexture = (ISdlGpuTexture)source;

        Render(_renderer.CommandBuffer,
            sdlSourceTexture.GetMap(TextureMaps.Diffuse),
            sdlSourceTexture.Size.Width, sdlSourceTexture.Size.Height,
            sdlRenderTarget.Handle,
            (uint)sdlRenderTarget.Size.Width, (uint)sdlRenderTarget.Size.Height,
            sourceScale,
            new SDL_FColor { r = 0, g = 0, b = 0, a = 1 });
