...
/// </summary>
public sealed unsafe class GlowEffect : IDisposable
{
    ...
    public GlowEffect(SDL_GPUDevice* device, SDL_GPUTextureFormat format, int glowWidth, int glowHeight)
    {
        _device = device;

        _blurPipeline = CreateOffscreenPipeline(device, format);
        _blitPipeline = CreateCompositePipeline(device, format, enableBlend: false);
        _additivePipeline = CreateCompositePipeline(device, format, enableBlend: true);
        ...
        int canvasWidth = glowWidth + (int)(BlurPadding * 2);
        int canvasHeight = glowHeight + (int)(BlurPadding * 2);
        ...
        _blurTextureA = SDL_CreateGPUTexture(device, &blurTextureCreateInfo);
        _blurTextureB = SDL_CreateGPUTexture(device, &blurTextureCreateInfo);
        ...
        float insetRight = (BlurPadding + glowWidth) / canvasWidth * 2f - 1f;
        float insetTop = 1f - BlurPadding / canvasHeight * 2f;
        float insetBottom = 1f - (BlurPadding + glowHeight) / canvasHeight * 2f;
        ...
        var compositeUvRight = (BlurPadding + glowWidth) / canvasWidth;
        var compositeUvTop = BlurPadding / canvasHeight;
        var compositeUvBottom = (BlurPadding + glowHeight) / canvasHeight;
        ...
        _fullscreenVertexBuffer = CreateAndUploadVertexBuffer(device, copyPass, fullscreenVertices);
        _insetVertexBuffer = CreateAndUploadVertexBuffer(device, copyPass, insetVertices);
        _glowCompositeVertexBuffer = CreateAndUploadVertexBuffer(device, copyPass, glowCompositeVertices);

        SDL_EndGPUCopyPass(copyPass);
        SDL_SubmitGPUCommandBuffer(uploadCommandBuffer);
    }

    public void Render(SDL_GPUCommandBuffer* commandBuffer, SDL_GPUTexture* regularTexture, SDL_GPUTexture* glowTexture, SDL_GPUTexture* outputTexture, float intensity)
    {
    ...
    private static SDL_GPUGraphicsPipeline* CreateOffscreenPipeline(SDL_GPUDevice* device, SDL_GPUTextureFormat targetFormat)
    {
        SDL_GPUShader* vertexShader = GpuShader.CreateFromEmbeddedResource(device, "Shaders.FullscreenQuad.vert.metal", "vertexMain", SDL_GPUShaderStage.SDL_GPU_SHADERSTAGE_VERTEX);
        SDL_GPUShader* fragmentShader = GpuShader.CreateFromEmbeddedResource(device, "Shaders.Blur.frag.metal", "fragmentMain", SDL_GPUShaderStage.SDL_GPU_SHADERSTAGE_FRAGMENT, numSamplers: 1, numUniformBuffers: 1);
        ...
        SDL_GPUGraphicsPipeline* pipeline = CreatePipeline(device, vertexShader, fragmentShader, targetFormat, enableBlend: false);
    ...
    private static SDL_GPUGraphicsPipeline* CreateCompositePipeline(SDL_GPUDevice* device, SDL_GPUTextureFormat targetFormat, bool enableBlend)
    {
        SDL_GPUShader* vertexShader = GpuShader.CreateFromEmbeddedResource(device, "Shaders.FullscreenQuad.vert.metal", "vertexMain", SDL_GPUShaderStage.SDL_GPU_SHADERSTAGE_VERTEX);
        SDL_GPUShader* fragmentShader = GpuShader.CreateFromEmbeddedResource(device, "Shaders.GlowComposite.frag.metal", "fragmentMain", SDL_GPUShaderStage.SDL_GPU_SHADERSTAGE_FRAGMENT, numSamplers: 1, numUniformBuffers: 1);
        ...
        SDL_GPUGraphicsPipeline* pipeline = CreatePipeline(device, vertexShader, fragmentShader, targetFormat, enableBlend);
