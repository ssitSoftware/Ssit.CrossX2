...
public unsafe class TextureGpuPipeline : IGpuPipeline2D
{
    ...
    public TextureGpuPipeline(SDL_GPUDevice* device, SDL_Window* window)
        : this(device, window, "Shaders.Screen.vert.metal", "Shaders.Screen.frag.metal", fragmentUniformBuffers: 0)
    {
    }

    protected TextureGpuPipeline(SDL_GPUDevice* device, SDL_Window* window, string vertexShaderResource, string fragmentShaderResource, int fragmentUniformBuffers)
    {
        _device = device;
    ...
    public virtual void Bind(SDL_GPUCommandBuffer* commandBuffer, SDL_GPURenderPass* renderPass, SDL_GPUTexture*[] textures)
    {
        ...
        {
            ScreenSize = new Vector4(ScreenSize.Width, ScreenSize.Height, 0f, 0f),
        ...
        var samplerBindings = stackalloc SDL_GPUTextureSamplerBinding[textures.Length];
        for (int i = 0; i < textures.Length; i++)
        {
            samplerBindings[i] = new SDL_GPUTextureSamplerBinding { texture = textures[i], sampler = Sampler };
        }
