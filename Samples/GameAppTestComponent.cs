...
public class GameAppTestComponent(IRenderer renderer, IContentManager contentManager, IAppTimer timer, IRenderHost host) : IAppComponent
{
    ...
    public void Draw()
    {
        ...
        _lights[0] = new PointLight(new Vector3(host.LogicalSize.ToVector() / 2f - new Vector2(50, 0), -256f), 128, RgbaColor.DarkViolet, 0f);
        _lights[1] = new PointLight(new Vector3(host.LogicalSize.ToVector() / 2f + Vector2.Transform(new Vector2(96), Matrix3x2.CreateRotation(-timer.RunTime * 2)), -512), 192, RgbaColor.GreenYellow, 1);

        _spotLights[0] = new SpotLight(new Vector3(host.LogicalSize.ToVector() / 2f - new Vector2(0,  _sampleTexture.Resource.Size.Height * 0.25f), -50),
        ...
        renderer.SpriteRenderer.Draw(
            ...
            host.LogicalSize.ToVector() / 2 - new Vector2(_sampleTexture.Resource.Size.Width / 2f * 0.35f, 0),
            null,
